...
def build_model(normalize=None):
    ...
    z = tf.keras.layers.concatenate([x, y], axis=1, name="fc_layer")
    z = layers.Dropout(0.2)(z)
    z = layers.Dense(1, activation="sigmoid")(z)
    
    #new_model = tf.keras.Model(model.layers[0].input, z)
    new_model = tf.keras.Model(model.input, z)
...
def doFFT(input):
    ...
    input = tf.transpose(input, perm=[0, 3, 1, 2])
    input = tf.signal.fft2d(input)
    input = tf.transpose(input, perm=[0, 2, 3, 1])
    input = tf.math.abs(input)
