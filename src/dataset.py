...
def deserialize_data(raw_record, raw=True, shape=[299, 299, 1]):
...
def deserialize_data_(raw_record, raw=True, shape=[299, 299, 1]):
    """Deserialize single tfrecord."""
    import tensorflow as tf
    IMAGE_FEATURE_DESCRIPTION = {
        'image': tf.io.FixedLenFeature(shape, tf.float32),
...
def serialize_data(data):
    ...
    # serialized_feature = proto.SerializeToString()
    # decoded_msg = tf.train.Example.FromString(serialized_feature)
